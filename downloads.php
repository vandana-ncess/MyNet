    ...
    <div id="templatemo_main"><span id="main_top"></span><span id="main_bottom"></span>
        ...
        <div id="templatemo_content">
            <ul class="breadcrumb" style="padding-top: 2px;">
                ...
                <li>Requisition Forms</li>
            </ul>
        	<div class="content_box" style="padding-bottom: 180px;padding-top: 50px;">
                    <h4>General Requisition Forms</h4>
                   
                        <?php
                            $sql = "SELECT * FROM general_forms";
                            $result = mysqli_query($conn,$sql);
                            if(mysqli_num_rows($result)>0) {
                                echo ' <ul class="llink">';
                                while($row = mysqli_fetch_array($result)) {
                                    echo '<li><a href="documents/forms/' . $row['fileName'] . '" target="_blank">' . $row['title'] . '</a></li>' ;
                                }
                                echo '</ul>';
                            }
                        ?>
                  
                <h4>Lab Analysis Requisition Forms</h4> 
                <?php
                            $sql = "SELECT * FROM lab_forms";
                            $result = mysqli_query($conn,$sql);
                            if(mysqli_num_rows($result)>0) {
                                echo ' <ul class="llink">';
                                while($row = mysqli_fetch_array($result)) {
