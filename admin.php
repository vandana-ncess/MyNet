...
<div id="templatemo_wrapper">
        	...
        	<div class="content_box" style="height : 550px; overflow-x:   scroll;overflow-y: auto;" id="search">
                    <?php
                        ...
                        if(mysqli_num_rows($result)>0) {
                            echo '<table><tr>';
                            echo '<td style="font-size:18px;padding-bottom:10px;" colspan="3">Chief Manager</td></tr><tr><td ><a href="employeeProfile.php?empID=1517" target="_blank" ><img class="image_wrapper image_fl"  width="140px" height="160px" src="images/profile/1517.jpg" alt="Image 1" /><b>Dr.Suresh Babu.D.S <br />Scientist F & Chief Manager (In-charge)</b></a></td></tr><tr>';
                            $row=mysqli_fetch_array($result);
                            $last = $row['divisionName'];
                             echo '<td style="font-size:18px;padding-bottom:10px;" colspan="3">' . $row['divisionName'] . '</td></tr><tr><td ><a href="employeeProfile.php?empID=' . $row['employeeCode'] . '" target="_blank" ><img class="image_wrapper image_fl"  width="140px" height="160px" src="images/profile/'.$row['employeeCode'] . '.jpg" alt="Image 1" /><b>' . $row['employeeName'] .'<br />' . $row['designation']. '</b></a></td>';
                             $i++;
                             while($row=mysqli_fetch_array($result)) {
                               if($last == $row['divisionName']) {
                                   ...
                                   echo '<tr><td  style="font-size:18px;padding-bottom:10px;padding-top:10px;" colspan="3">' . $row['divisionName'] . '</td></tr><tr>';
                                   $i = 1;
                                   echo '<td><a href="employeeProfile.php?empID=' . $row['employeeCode'] . '" target="_blank" ><img class="image_wrapper image_fl" src="images/profile/'.$row['employeeCode'] . '.jpg" alt="Image 1" width="140px" height="160px" /><b>' . $row['employeeName'] .'<br />' . $row['designation']. '</b></a></td>';
                                    if(($i%3)==0)
                                        echo '</tr><tr>';
                        ...
                        $sql = "SELECT * FROM employee WHERE categoryID=5 AND designationID = 23 ORDER BY employeeCode";
                        $result = mysqli_query($conn,$sql);
                        $i=1;
                        if(mysqli_num_rows($result)>0) {
                            echo '</tr><tr><td style="font-size:18px;padding-bottom:10px;padding-top:10px;" colspan="3">MTS</td></tr><tr>';
                            while($row=mysqli_fetch_array($result)) {
                               echo '<td><a href="employeeProfile.php?empID=' . $row['employeeCode'] . '" target="_blank" ><img class="image_wrapper image_fl" src="images/profile/'.$row['employeeCode'] . '.jpg" alt="Image 1" width="140px" height="160px" /><b>' . $row['employeeName'] . '</b></a></td>';
                                    if(($i%3)==0)
                                        echo '</tr><tr>';
                                   $i++;
                               }
    ...
    <script type="text/javascript">
        function search() {
            ...
            var uri = "getStaff.php?category=admin&str="+ $str;
