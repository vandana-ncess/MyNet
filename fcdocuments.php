        ...
        <div id="templatemo_content">
        	...
        	<div style="padding-top: 50px;padding-left: 20px;overflow-y: scroll;min-height:650px;" >
                <div style='float: left;width:300px;'>
                    <?php
                            $sql = "SELECT * FROM fc_meetings where year=" . $yr;
                            $result = mysqli_query($conn,$sql);
                            $i=1;
                            if(mysqli_num_rows($result)>0) {
                                echo '<table>';
                                while($row=mysqli_fetch_array($result)) {
                                    echo '<tr><td><a href="documents/fc/' . $row['fileName'] . '" target="_blank" ><h5>' . $row['title'] .'</h5><img class="image_wrapper image_fl" src="images/pdf.ico" alt="Image 1" /></a></td>';
                                        echo '</tr>';
                                }echo '</table>';
                            }
