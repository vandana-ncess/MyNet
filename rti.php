    ...
    <div id="templatemo_main"><span id="main_top"></span><span id="main_bottom"></span>
        ...
        <div id="templatemo_content">
        	...
        	<div class="content_box" style="padding-bottom: 180px;padding-top: 50px;">
                    <?php
                        $sql = "SELECT * FROM rti where year=". date('Y');
                        $result = mysqli_query($conn,$sql);
                        $i=1;
                        if(mysqli_num_rows($result)>0) {
                            echo '<table id="tblDocument"><tr>';
                            while($row=mysqli_fetch_array($result)) {
                                echo '<td style="padding-right:20px;"><a href="documents/rti/' . $row['fileName'] . '" target="_blank" ><h5>' . $row['title'] .'</h5><img class="image_wrapper image_fl" src="images/file.ico" alt="Image 1" /></a></td>';
                                if(($i%5)==0)
                                    echo '</tr><tr>';
...
<script type="text/javascript">
        function loadCirculars()
        {
            ...
            xmlhttp.open("GET","getCirculars.php?type=rti&str="+$str);
            xmlhttp.send();
        }
    </script>
</body>
