<?php 
   ...
   if (isset($_SESSION['LAST_ACTIVITY']) && (time() - $_SESSION['LAST_ACTIVITY'] > 1800)) {
    ...
    $sql = "SELECT * FROM oc_committee where committeeID=" . $grp;
    $result = mysqli_query($conn,$sql);
...
<div id="templatemo_wrapper">
    ...
    <div id="templatemo_main"><span id="main_top"></span><span id="main_bottom"></span>
        ...
        <div id="templatemo_content">
        	...
        	<div style="padding-top: 50px;overflow-y: scroll;min-height:650px;" >
                <div style='float: left;width:500px;'>
            	<?php
                            $sql = "SELECT * FROM oc_meetings where ocCommitteeID=" . $grp . " AND year=" . date('Y');
                            $result = mysqli_query($conn,$sql);
                            if(mysqli_num_rows($result)>0) {
                                echo '<table  id="tblDocument">';
                                $i=1;
                                while($row=mysqli_fetch_array($result)) {
                                    echo '<tr><td><a href="documents/oc/' . $row['fileName'] . '" target="_blank" ><h5>' . $row['title'] .'</h5><img class="image_wrapper image_fl" src="images/pdf.ico" alt="Image 1" /></a></td>';
                                    if($i%5==0)
                                echo "</tr><tr>";
                                $i++;    
                                } 
